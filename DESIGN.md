...
- Code blocks use a Codex-inspired warm charcoal surface (`#242424`) with a compact `</>` language header, icon-only Copy action, rounded 20px public container, and horizontally scrollable monospace content. Selected HTML, CSS, JavaScript, TypeScript, JSON, and Shell grammars receive server-safe syntax highlighting; the admin live preview uses the same token palette. They inherit the current Content width setting, support an optional muted caption below the code panel, and remain responsive on mobile.
